    ...
    if (this.splitted.length > 1) {
      if (this.splitted[1].length > 1) {
        ...
        if (fraction >= 1 && fraction <= 99) {
          ...
          for (
            let index = 0;
            index < currencies[currency as keyof typeof currencies].decimals;
            index++
          ) {
      ...
      if (this.fraction != 0) {
        if (this.digit >= 3 && this.digit <= 10) {
          str +=
            ...
            " " +
            currencies[this.currency as keyof typeof currencies].fractions;
        } else {
          str +=
            " و" +
            this.read(this.fraction) +
            " " +
            currencies[this.currency as keyof typeof currencies].fraction;
        }
      }
    }
  ...
  private readHundreds(d: number) {
    ...
    if (Array.from(d.toString())[1] !== "0") {
      str +=
        " و" +
        this.readTens(
          parseInt((Array.from(d.toString())[1] + Array.from(d.toString())[2]).toString(), 10)
        );
    }
    return str;
  };
  ...
  private addSuffixPrefix(arr: string[], columnIdx: number) {
    ...
    if (arr.length == 1) {
      ...
      if (parseInt(arr[0]) > 2 && parseInt(arr[0]) <= 9) {
        if(columnConstant)
          return (
            `${this.readOnes(parseInt(arr[0]))} ${columnConstant.plural}`
          );
      }
    } else {
      ...
      if (joinedNumber > 1) {
        if(columnConstant)
          return (
            `${this.read(joinedNumber)} ${columnConstant.singular}`
          );
      } else {
        if(columnConstant)
          return columnConstant.singular;
