...
class Tafgeet {
    ...
    parse() {
        ...
        if (this.currency !== '') {
            ...
            if (this.fraction !== 0) {
                if (this.digit >= 3 && this.digit <= 10) {
                    str += ' و' + this.read(this.fraction) + ' ' + constants_1.currencies[this.currency].fractions;
                }
                else {
                    str += ' و' + this.read(this.fraction) + ' ' + constants_1.currencies[this.currency].fraction;
                }
            }
        }
        str += ' فقط لا غير';
    ...
    addSuffixPrefix(arr, columnIdx) {
        ...
        if (arr.length === 1) {
            ...
            if (parseInt(arr[0], 10) > 2 && parseInt(arr[0], 10) <= 9) {
                if (columnConstant)
                    return `${this.readOnes(parseInt(arr[0], 10))} ${columnConstant.plural}`;
            }
        }
        else {
